    ...
    <div className="min-h-screen" style={{ background: cream, color: dark }}>
      ...
      <section className="px-6 py-8">
        <div className="mx-auto max-w-7xl rounded-3xl border p-4 sm:p-6 shadow-sm" style={{ background: "#fff", borderColor: `${sage}55` }}>
          ...
          <div id="city-no-results" className="rounded-xl border border-dashed p-8 text-center text-muted-foreground" style={{ display: "none" }}>
            No cities match your search.
          </div>

          <div className="space-y-12">
            {letters.map((letter) => (
              <div key={letter} id={`city-group-${letter}`} data-city-group={letter} className="scroll-mt-32">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                  {groupedCities[letter].map((city) => (
                    <Link key={city.name} href={`/daycares/${city.slug}`} className="group block h-full" data-city-name={city.name}>
